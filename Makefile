...
REPOSITORY ?= packyzbq/prometheus
NAME       := busybox
...
.PHONY: build
build:
	@./build.sh "$(REPOSITORY)/$(NAME)-linux-mips64le" "" "$(SUFFIX)" $(VERSIONS)
...
.PHONY: tag
tag:
	docker tag "$(REPOSITORY)/$(NAME)-linux-mips64le:uclibc" "$(REPOSITORY)/$(NAME)-linux-mips64le:latest"
